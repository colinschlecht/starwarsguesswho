...
questionLI.innerText =
	"PRESS ARROW KEY TO CHANGE CATEGORY/QUESTION, ENTER TO SELECT";
...
function questionControlFlow(question) {
	...
	switch (question.category.id) {
		...
		case 3:
			...
			break;
		case 4:
			if (mysteryCharacter.height == 0) {
				answer = false;
			} else if (
				question.attribute_desc == 100 &&
				mysteryCharacter.height <= 100
			) {
				answer = true;
			} else if (
				question.attribute_desc == 200 &&
				mysteryCharacter.height >= 200
			) {
				answer = true;
			} else if (question.attribute_desc == 0 && mysteryCharacter.height != 0) {
				answer = true;
			}
		...
		case 6:
			if (
				mysteryCharacter.hair_color === "none" &&
				question.attribute_desc === "none"
			) {
				answer = false;
			} else if (mysteryCharacter.hair_color == question.attribute_desc) {
				answer = true;
			}
			break;
		case 7:
			if (mysteryCharacter.mass == 0) {
				answer = false;
			} else if (question.attribute_desc == 50 && mysteryCharacter.mass < 50) {
				answer = true;
			} else if (question.attribute_desc == 0 && mysteryCharacter.mass != 0) {
                answer = true
			} else if (question.attribute_desc == 50 && mysteryCharacter.mass < 50) {
				answer = true;
			} else if (question.attribute_desc == 75 && mysteryCharacter.mass > 75) {
				answer = true;
			}
		...
		case 8:
			if (
				mysteryCharacter.homeworld.name === "unknown" &&
				question.attribute_desc === "unknown"
			) {
				answer = false;
			} else if (mysteryCharacter.homeworld.name == question.attribute_desc) {
				answer = true;
			}
			break;
		case 9:
			...
			break;
		case 10:
			if (mysteryCharacter.birth_year == 0) {
				answer = false;
			} else if (question.attribute_desc == 0) {
				answer = true;
			} else if (
				question.attribute_desc == 50 &&
				mysteryCharacter.birth_year <= 50
			) {
				answer = true;
			} else if (
				question.attribute_desc == 100 &&
				mysteryCharacter.birth_year <= 100
			) {
				answer = true;
			} else {
				answer = false;
...
function handleAnswer(answer, question) {
	...
	if (question.category.name == "Name" && answer == true) {
		...
		scratchA.innerText = "Hello There! - aka yes";
	} else {
		scratchA.innerText = "* visible confustion * - aka no";
	}
